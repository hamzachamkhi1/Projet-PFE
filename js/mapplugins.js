  ...
  (InfoBox.prototype.panBox_ = function (t) {
    ...
    if (!t && (i = this.getMap()) instanceof google.maps.Map) {
      ...
      var s = i.getDiv(),
        ...
        _ = this.infoBoxClearance_.width,
        p = this.infoBoxClearance_.height,
        v = this.getProjection().fromLatLngToContainerPixel(this.position_);
      if (
        (v.x < -l + _
          ? (e = v.x + l - _)
          : v.x + r + l + _ > n && (e = v.x + r + l + _ - n),
        this.alignBottom_
          ? v.y < -d + p + a
            ? (o = v.y + d - p - a)
            : v.y + d + p > h && (o = v.y + d + p - h)
          : v.y < -d + p
          ? (o = v.y + d - p)
          : v.y + a + d + p > h && (o = v.y + a + d + p - h),
        0 !== e || 0 !== o)
      ) {
        i.getCenter();
        i.panBy(e, o);
  ...
  (InfoBox.prototype.setBoxStyle_ = function () {
    ...
    if (this.div_) {
      (this.div_.className = this.boxClass_),
        ...
        (i = this.boxStyle_);
      for (t in i) i.hasOwnProperty(t) && (this.div_.style[t] = i[t]);
